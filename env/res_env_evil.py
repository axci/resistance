...
class ResistanceEvilEnv:
    def __init__(self):
        super(ResistanceEvilEnv).__init__()
        """
        PHASES:    
            (0) Team Building
            (1) Team Voting
    ...
    def reset(self):
        ...
        obs = { agent_id: self._get_obs(agent_id) for agent_id in self.evil_players }
        available_actions = { agent_id: self._get_available_actions(agent_id) for agent_id in self.evil_players }
    ...
    def step(self, action_dict, verbose=False):
        rewards = {
            agent_id: 0 for agent_id in self.evil_players
        }
        ...
        if self.phase == 0:
            ...
            if vote_action == 0:  # if Quest succeeded
                rewards = { agent_id: -self.reward_completed_quest_bonus for agent_id in self.evil_players }
            else:
                rewards = { agent_id: self.reward_completed_quest_bonus for agent_id in self.evil_players }
        ...
        if self.done:
            if self.good_victory:
                rewards = { agent_id: -self.reward_win_bonus for agent_id in self.evil_players }
            else:
                rewards = { agent_id: self.reward_win_bonus for agent_id in self.evil_players }
        ...
        rewards_array = np.reshape(self._convert_dict_2d_array(rewards), (self.num_evil, 1)  )
    ...
    def _get_available_actions(self, agent_id) -> np.ndarray:
        ...
        if self.leader == agent_id and self.phase == 0:
            ...
            if agent_id in self.good_players:
                mask[23] = 1
            else:
                mask[23:25] = [1] * 2
        else:
