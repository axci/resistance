...
class NaiveEvilPolicy():
    ...
    def get_actions(self, share_obs: np.ndarray, obs: np.ndarray, available_actions):
        ...
        if available_actions[1] == 1: # if I am leader and the Phase is 0 and team size is 2
            ...
            for round_hist in hist:
                if sum(round_hist) != 0:
                    team_to_vote_for = self._find_first_5_elements_with_multiple_ones(round_hist)
                    break
            chosen_action = 22
    ...
    def _find_first_5_elements_with_multiple_ones(self, lst):
...
class RandomPolicy():
    ...
    def get_actions(self, share_obs: np.ndarray, obs: np.ndarray, available_actions):
        ...
        if available_actions[1] == 1: # if I am leader and the Phase is 0 and team size is 2            
            ...
            chosen_action  = random.choice(range(11, 21))
        
        elif available_actions[21] == 1: # Team Voting
            chosen_action  = random.choice([21, 22])            
        
        elif available_actions[23] == 1:  # Quest Voting
            if len(obs) == 425:
                chosen_action  = random.choice([23, 24])
            else:
                chosen_action  = 23
        else:
            chosen_action = 0
