...
class ResistanceFinalEnv:
    def __init__(self):
        super(ResistanceFinalEnv).__init__()
        """
        PHASES:    
            (0) Team Building
            (1) Team Voting
    ...
    def reset(self):
        ...
        return obs, share_obs, available_actions, info #, share_obs, available_actions, info
    
    def step(self, action_dict, verbose=False):
        rewards = { agent_id: 0 for agent_id in self._agent_ids }
        ...
        if self.phase == 0:
            ...
            if vote_action == 0:  # if Quest succeeded
                rewards = { 
                    agent_id: self.reward_completed_quest_bonus if agent_id in self.good_players else -self.reward_completed_quest_bonus for agent_id in self._agent_ids 
                    }
            else:
                rewards = { 
                    agent_id: -self.reward_completed_quest_bonus if agent_id in self.good_players else self.reward_completed_quest_bonus for agent_id in self._agent_ids 
        ...
        #rewards_array = np.reshape(self._convert_dict_2d_array(rewards), (self.num_players, 1)  )
        return obs, share_obs, available_actions, rewards, self.done, info
    ...
    def _get_obs(self, agent_id: str) -> list:
        ...
        # Return different obs for different roles:
        if agent_id in self.evil_players:
            return idx + history_flattened + knowledge
        else:
            return idx + history_flattened

    def _get_share_obs(self, agent_id: str) -> np.ndarray:
        ...
        info = {
            agent_id: {
               ...
               'round 2': {
                ...
                'knowledge': obs[agent_id][420:425],
                    },
            } for agent_id in self.evil_players
            }
        return info
